 ...
 * @returns
 */
var scoreRules = function (chartType, dataProps, ruleBase, options) {
    var exportLog = options === null || options === void 0 ? void 0 : options.exportLog;
    /** @deprecated */
    var showLog = options === null || options === void 0 ? void 0 : options.showLog;
    ...
    var hardScore = computeScore(ruleBase, 'HARD', info, log);
    // Hard-Rule pruning
    ...
    //   return result;
    // }
    var softScore = computeScore(ruleBase, 'SOFT', info, log);
    /** @since 3.0.0 @todo score normalization  */
    // proposal:
    // const score = hardScore * 100 * (softFullScore ? softScore / softFullScore : 0);
    var score = hardScore * (1 + softScore);
    // eslint-disable-next-line no-console
    if (showLog)
        console.log('💯score: ', score, '=', hardScore, '* (1 +', softScore, ') ;charttype: ', chartType);
    // eslint-disable-next-line no-console
    if (showLog)
        console.log(log);
    var result = { chartType: chartType, score: score };
    if (exportLog)
        result.log = log;
    return result;
};
...
function applyTheme(dataProps, chartSpec, theme) {
    ...
    var layerEnc = 'encoding' in chartSpec.layer[0] ? chartSpec.layer[0].encoding : null;
        ...
        if (layerEnc.color) {
            ...
            return {
                encoding: {
                    color: {
                        scale: {
                            range: palette.colors.map(function (color) { return colorToHex(color); }),
                        },
                    },
                },
            };
...
export function dataToAdvices(data, dataProps, chartWIKI, ruleBase, smartColor, options, colorOptions) {
     ...
     */
    var exportLog = (options === null || options === void 0 ? void 0 : options.exportLog) || false;
    /**
    ...
    var list = CHART_ID_OPTIONS.map(function (t) {
        ...
        var resultForChartType = scoreRules(t, dataProps, ruleBase, options);
