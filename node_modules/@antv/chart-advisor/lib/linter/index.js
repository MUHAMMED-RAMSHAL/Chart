...
var Linter = /** @class */ (function () {
    function Linter(ruleCfg) {
        this.ruleBase = ruler_1.processRuleCfg(ruleCfg);
    }
    /**
    ...
    Linter.prototype.checkRules = function (params) {
        ...
        var chartType = utils_1.getChartType(spec);
        var lints = [];
        // for log
        var log = [];
        if (!spec || !chartType) {
        ...
        if (!dataProps || !dataProps.length) {
            ...
            try {
                dataFrame = new data_wizard_1.DataFrame(spec.data.values);
        ...
        // step 2: lint rules
        // HARD and SOFT rules
        utils_1.lintRules(this.ruleBase, 'notDESIGN', info, log, lints);
        // DESIGN rules
        utils_1.lintRules(this.ruleBase, 'DESIGN', info, log, lints, spec);
