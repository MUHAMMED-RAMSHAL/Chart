...
exports.getChartTypeSpec = void 0;
...
function splitAngleColor(dataProps) {
    var field4Color = dataProps.find(function (field) { return utils_1.hasSubset(field.levelOfMeasurements, ['Nominal']); });
    var field4Angle = dataProps.find(function (field) { return utils_1.hasSubset(field.levelOfMeasurements, ['Interval']); });
    return [field4Color, field4Angle];
}
function pieChart(data, dataProps) {
    var _a = tslib_1.__read(splitAngleColor(dataProps), 2), field4Color = _a[0], field4Angle = _a[1];
...
function splitLineXY(dataProps) {
    var field4X = dataProps.find(function (field) { return utils_1.intersects(field.levelOfMeasurements, ['Time', 'Ordinal']); });
    var field4Y = dataProps.find(function (field) { return utils_1.hasSubset(field.levelOfMeasurements, ['Interval']); });
    var field4Color = dataProps.find(function (field) { return utils_1.hasSubset(field.levelOfMeasurements, ['Nominal']); });
    return [field4X, field4Y, field4Color];
}
function lineChart(data, dataProps) {
    var _a = tslib_1.__read(splitLineXY(dataProps), 3), field4X = _a[0], field4Y = _a[1], field4Color = _a[2];
    if (!field4X || !field4Y)
        return null;
    var spec = {
        basis: { type: 'chart' },
...
function splitAreaXYSeries(dataProps) {
    var field4X = dataProps.find(function (field) { return utils_1.intersects(field.levelOfMeasurements, ['Time', 'Ordinal']); });
    var field4Series = dataProps.find(function (field) { return utils_1.hasSubset(field.levelOfMeasurements, ['Nominal']); });
    var field4Y = dataProps.find(function (field) { return utils_1.hasSubset(field.levelOfMeasurements, ['Interval']); });
...
function stackedAreaChart(data, dataProps) {
    var _a = tslib_1.__read(splitAreaXYSeries(dataProps), 3), field4X = _a[0], field4Y = _a[1], field4Series = _a[2];
...
function splitBarXYSeries(dataProps) {
    var nominalFields = dataProps.filter(function (field) { return utils_1.hasSubset(field.levelOfMeasurements, ['Nominal']); });
    var sortedNominalFields = nominalFields.sort(utils_2.compare);
    var field4Y = sortedNominalFields[0];
    var field4Series = sortedNominalFields[1];
    var field4X = dataProps.find(function (field) { return utils_1.hasSubset(field.levelOfMeasurements, ['Interval']); });
    return [field4X, field4Y, field4Series];
}
...
function groupedBarChart(data, dataProps) {
    var _a = tslib_1.__read(splitBarXYSeries(dataProps), 3), field4X = _a[0], field4Y = _a[1], field4Series = _a[2];
    if (!field4X || !field4Y || !field4Series)
        return null;
    var spec = {
        basis: { type: 'chart' },
...
function splitColumnXYSeries(dataProps) {
    ...
    var field4X;
    var Field4Series;
    if (data_wizard_1.utils.isParentChild((_c = sortedNominalFields[1]) === null || _c === void 0 ? void 0 : _c.rawData, (_d = sortedNominalFields[0]) === null || _d === void 0 ? void 0 : _d.rawData)) {
        _a = tslib_1.__read(sortedNominalFields, 2), Field4Series = _a[0], field4X = _a[1];
    }
    else {
        _b = tslib_1.__read(sortedNominalFields, 2), field4X = _b[0], Field4Series = _b[1];
    }
    var field4Y = dataProps.find(function (field) { return utils_1.hasSubset(field.levelOfMeasurements, ['Interval']); });
    return [field4X, field4Y, Field4Series];
}
...
function groupedColumnChart(data, dataProps) {
    var _a = tslib_1.__read(splitColumnXYSeries(dataProps), 3), field4X = _a[0], field4Y = _a[1], Field4Series = _a[2];
    if (!field4X || !field4Y || !Field4Series)
        return null;
    var spec = {
        basis: { type: 'chart' },
...
function scatterPlot(data, dataProps) {
    var intervalFields = dataProps.filter(function (field) { return utils_1.hasSubset(field.levelOfMeasurements, ['Interval']); });
    var sortedIntervalFields = intervalFields.sort(utils_2.compare);
...
function bubbleChart(data, dataProps) {
    ...
    var _loop_1 = function (i) {
        var _loop_2 = function (j) {
            var p = data_wizard_1.statistics.pearson(intervalFields[i].rawData, intervalFields[j].rawData);
            if (Math.abs(p) > triple.corr) {
                triple.x = intervalFields[i];
                triple.y = intervalFields[j];
                triple.corr = p;
                triple.size = intervalFields[tslib_1.__spreadArray([], tslib_1.__read(Array(intervalFields.length).keys())).find(function (e) { return e !== i && e !== j; }) || 0];
            }
        };
    ...
    var field4Color = dataProps.find(function (field) { return utils_1.intersects(field.levelOfMeasurements, ['Nominal']); });
...
function histogram(data, dataProps) {
    var field = dataProps.find(function (field) { return utils_1.hasSubset(field.levelOfMeasurements, ['Interval']); });
    if (!field)
        return null;
    var spec = {
        basis: { type: 'chart' },
...
function heatmap(data, dataProps) {
    var axisFields = dataProps.filter(function (field) { return utils_1.intersects(field.levelOfMeasurements, ['Nominal', 'Ordinal']); });
    var sortedFields = axisFields.sort(utils_2.compare);
    var field4X = sortedFields[0];
    var field4Y = sortedFields[1];
    var field4Color = dataProps.find(function (field) { return utils_1.hasSubset(field.levelOfMeasurements, ['Interval']); });
 ...
 */
function getChartTypeSpec(chartType, data, dataProps, chartKnowledge) {
    // step 0: check whether the chartType is default in `ChartID`
    // if not, use customized `toSpec` function
    if (!ckb_1.CHART_ID_OPTIONS.includes(chartType) && chartKnowledge) {
...
exports.getChartTypeSpec = getChartTypeSpec;
