...
var DataFrame = /** @class */ (function (_super) {
        ...
        if (isArray(data)) {
            ...
            if (isObject(data0)) {
                ...
                for (var i = 0; i < data.length; i += 1) {
                    var datum = data[i];
                    keys.push.apply(keys, __spreadArray([], __read(Object.keys(datum))));
                }
                var columns = __spreadArray([], __read(new Set(keys)));
                for (var i = 0; i < data.length; i += 1) {
                    var datum = data[i];
    ...
    DataFrame.prototype.generateColumns = function (columns, extraColumns) {
        if (extraColumns) {
            assert((extraColumns === null || extraColumns === void 0 ? void 0 : extraColumns.length) === columns.length, "Columns length is " + (extraColumns === null || extraColumns === void 0 ? void 0 : extraColumns.length) + ", but data column is " + columns.length);
    ...
    DataFrame.prototype.info = function () {
        ...
        for (var i = 0; i < ((_a = this.columns) === null || _a === void 0 ? void 0 : _a.length); i += 1) {
            ...
            fields.push(__assign(__assign({}, analyzeField(this.colData[i])), { name: String(column) }));
        }
        return fields;
    };
    /**
    ...
    DataFrame.prototype.toString = function () {
        ...
        return "" + generateSplit(maxLengths[0]) + this.columns
            ...
            return "" + col + (i !== _this.columns.length ? generateSplit(maxLengths[i + 1] - getStringifyLength(col) + 2) : '');
        })
            .join('') + "\n" + this.indexes
            .map(function (idx, idxIndex) {
            var _a;
            return "" + idx + generateSplit(maxLengths[0] - getStringifyLength(idx)) + ((_a = _this.data[idxIndex]) === null || _a === void 0 ? void 0 : _a.map(function (datum, i) {
                return "" + stringify(datum) + (i !== _this.columns.length ? generateSplit(maxLengths[i + 1] - getStringifyLength(datum)) : '');
