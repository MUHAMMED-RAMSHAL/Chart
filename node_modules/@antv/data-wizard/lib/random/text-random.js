...
var TextRandom = /** @class */ (function (_super) {
    tslib_1.__extends(TextRandom, _super);
    function TextRandom() {
        var _this = _super !== null && _super.apply(this, arguments) || this;
        _this.database = database_1.getTextDB();
        return _this;
    }
    /**
    ...
    TextRandom.prototype.character = function (options) {
        var opts = utils_2.initOptions({ lower: true, upper: true, symbols: true, numeric: true }, options);
        var pool = '';
        if (opts.pool) {
            pool = opts.pool;
        }
    ...
    TextRandom.prototype.string = function (options) {
        var opts = utils_2.initOptions({ length: this.natural({ min: 5, max: 20 }) }, options);
        utils_1.assert(opts.length >= 0, 'Length cannot be less than zero.');
    ...
    TextRandom.prototype.syllable = function (options) {
        var length = utils_2.initOptions({ length: this.natural({ min: 2, max: 3 }) }, options).length;
    ...
    TextRandom.prototype.word = function (options) {
        ...
        utils_1.assert(!options.syllables || !options.length, 'Cannot specify both syllables AND length.');
    ...
    TextRandom.prototype.sentence = function (options) {
        var opts = utils_2.initOptions({ words: this.natural({ min: 12, max: 18 }), punctuation: true }, options);
    ...
    TextRandom.prototype.paragraph = function (options) {
        var sentences = utils_2.initOptions({ sentences: this.natural({ min: 3, max: 7 }) }, options).sentences;
    ...
    TextRandom.prototype.givenName = function (options) {
        var gender = utils_2.initOptions({}, options).gender;
        utils_1.assert(!gender || gender === 'female' || gender === 'male', 'Gender must be one of female or male');
    ...
    TextRandom.prototype.cCharacter = function (options) {
        var pool = utils_2.initOptions({}, options).pool;
    ...
    TextRandom.prototype.cWord = function (options) {
        var length = utils_2.initOptions({ length: this.natural({ min: 2, max: 6 }) }, options).length;
    ...
    TextRandom.prototype.cSentence = function (options) {
        var opts = utils_2.initOptions({ min: 10, max: 18 }, options);
    ...
    TextRandom.prototype.cParagraph = function (options) {
        var opts = utils_2.initOptions({ min: 3, max: 18 }, options);
        var length = this.natural(opts);
    ...
    TextRandom.prototype.cGivenName = function (options) {
        var _a = utils_2.initOptions({ length: this.natural({ min: 1, max: 2 }) }, options), length = _a.length, gender = _a.gender;
        utils_1.assert(!gender || gender === 'female' || gender === 'male', 'Gender must be one of female or male');
    ...
    TextRandom.prototype.cZodiac = function (options) {
        var locale = utils_2.initOptions({ locale: 'zh-CN' }, options).locale;
        var list = this.database.cZodiac[locale];
        return this.pickone(list);
    };
    return TextRandom;
}(basic_random_1.BasicRandom));
exports.TextRandom = TextRandom;
